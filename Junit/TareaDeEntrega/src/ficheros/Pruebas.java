...
public class Pruebas {
	...
	public static void cargaDatos(Consigna con) {
		...
		Empleado e2 = new Empleado("Maria","Remilgos", "84937023L", 638172937
									,"Jaimita@gmail.com",Genero.MUJER,19);
		Empleado e3 = new Empleado("Manuel","Franchesco", "98671223M", 692123417,Genero.HOMBRE,38);
		Cliente c1 = new Cliente("Marina","Ramirez", "623574890M", 73279873,"marina@gmail.com"
								,Genero.MUJER,67);
		anadirClientes(c1);
		Cliente c2 = new Cliente("Eric","Gonzalez", "348917289Ñ", 896432789,Genero.HOMBRE,36);
		anadirClientes(c2);
		Cliente c3 = new Cliente("Marisa","Meniscos", "372134678M", 985320654
								,"correoEstandar@gmail.com",Genero.MUJER,17);
		anadirClientes(c3);
		con.anadirElementos(c1, 1,"Reloj diamantino,mochila "
							+ "skitty,Gafas recfacheras",3, "03/03/2024");
		con.anadirElementos(c2, 3,"Ordenador Gygabyte",1, "05/03/2024");
		con.anadirElementos(c3, 2,"Mochila portatil,chaqueta cara",2, "03/03/2024");
		con.anadirElementos(e1);
		con.anadirElementos(e2);
		con.anadirElementos(e3);
			
	}
	/**
	 * Menu basico con las 8 opciones para el usuario, que debera escribir un numero 
	...
	public static void prepararEmpleado(Consigna con) {
		...
		con.anadirElementos(contenedor);
		...
		anadirClientes(contenedor);
		
		
	}
	
	/**
	 * Recibe un String que distingue entre hombre y mujer para establecerlo 
	 * como enumeracion de genero
	 * @param gender
	 ...
	 * @param Cliente : newC
	 */
	public static void anadirClientes(Cliente newC) {
	...
	/**
	 * Metodo que muestra una clase Cliente a partir de una id introducida, devuelve 
	 * un booleano indicando si ha sido
	 * encontrada
	 * @param String : id
	 * @return
	 */
	...
	public static void prepararConsignacion(Consigna con) {
		...
		System.out.println("De las ids escritas arriba escriba una para asociarla a la nueva "
							+ "consignacion: ");
		String id = keyboard.next();
		...
		System.out.println("Introduzca la fecha de ingreso en la consigna con el siguiente "
							+ "formato XX/XX/XXXX:");
		String fecha = keyboard.next();
		
		con.anadirElementos(localizarCliente(id),peso,descripcion,numObjetos,fecha);
	...
	public static void prepararEliminacion(Consigna con) {
		...
		System.out.println("\nA continuacion se le listara todos los datos de la consigna para "
							+ "que escoga la id a eliminar");
		...
		System.out.println("\n\n Nota: Recuerde, si elimina un cliente eso no significa"
							+ " que dejara de estar asociado a su consignacion, para borrar todo "
							+ "rastro del cliente debera borrar tambien la consignacion");
	...
	public static void setNumClientes(int numClientes) {
		...
		Pruebas.numClientes = numClientes;
	}
	...
	/**
	 * Metodo que devuelve la posicion donde se localiza el objeto cliente dentro 
	 * del vector localizado en la clase
	 * Pruebas
	 * @param Cliente : cBuscar
	 * @return
	 */
	 ...
	 * @param Consigna : con
	 */
	public static void prepararFechaSalida(Consigna con) {
		System.out.println("Escoge una consignacion a la que asignarle una "
							+ "fecha de extracion (si ya no ha sido introducida):");
		...
		if (id.charAt(0) == 'O') {
			...
			if (pos == -1) {
				...
				System.out.println("La id introducida no es valida o en su defecto es de otro "
									+ "registro");
				
			}else {
				System.out.println("Introduce la fecha de salida que quieras meterle a " 
									+ c[pos].getId());
				String FechaSalida = keyboard.next();
				con.actualizarFecha(pos,FechaSalida);
			}
			
		}else{
			System.out.println("La id introducida no es valida o en su defecto es "
								+ "de otro registro");
		}
